...
class RunningStat(object):
    ...
    def __init__(self, shape=None, mean=None, std=None):
        self._n = 0
        self._M = mean or np.zeros(shape, dtype = np.float64)
        self._S = std or np.zeros(shape,  dtype = np.float64)
...
class MeanStdFilter(Filter):
    ...
    def __init__(self, shape, demean=True, destd=True, mean=None, std=None):
        self.shape = shape
        self.demean = demean
        self.destd = destd
        self.rs = RunningStat(shape, mean=mean, std=std)
...
def get_filter(filter_config, shape=None, mean=None, std=None):
    if filter_config == "MeanStdFilter":
        return MeanStdFilter(shape, mean=mean, std=std)
