...
try:
    ...
    sys.path.append(
        glob.glob(
            r"D:\CARLA_0.9.13\PythonAPI\carla\dist\carla-*%d.%d-%s.egg"
            % (
...
from carla import ActorBlueprint, BlueprintLibrary, World, Actor
...
class CarEnv:
    def __init__(
        ...
        self.blueprint_library: BlueprintLibrary = self.world.get_blueprint_library()
        self.car: ActorBlueprint = self.blueprint_library.filter(car_model)[0]
        self.show_cam: bool = show_cam
        self.control_type: str = control_type
        self.front_camera: Union[np.ndarray, None] = None
        self.actor_list: List[Actor] = []
        self.seconds_per_episode: int = seconds_per_episode
        self.collision_hist: list = []
        self.steering_cache: list = []
    ...
    def reset(self) -> np.ndarray:
        ...
        # Attach RGB Camera
        self.rgb_cam: ActorBlueprint = self.blueprint_library.find("sensor.camera.rgb")
        ...
        self.rgb_sensor: Actor = self.world.spawn_actor(self.rgb_cam, transform, attach_to=self.vehicle)
        ...
        collision_sensor: ActorBlueprint = self.blueprint_library.find("sensor.other.collision")
        self.collision_sensor: Actor = self.world.spawn_actor(collision_sensor, transform, attach_to=self.vehicle)
    ...
    def process_img(self, image):
        img = np.array(image.raw_data).reshape((self.img_height, self.img_width, 4))[..., :3]
        if self.show_cam:
            cv2.imshow("", img)
            cv2.waitKey(1)
...
class Worker(object):
    ...
    def __init__(
        ...
        self.env = CarEnv(show_cam=show_cam, seconds_per_episode=seconds_per_episode)
    ...
    def rollout(self, shift=0.0, state_filter=False):
        """
        ...
        while not done:
            steps += 1
            rgb_img = self.env.front_camera.reshape((1, 224, 224, 3)) / 255.0
            state = self.base_model.predict(rgb_img).flatten() / 10
    ...
    def do_rollouts(
        ...
        for i in range(num_rollouts):
            ...
            if evaluate:
                ...
                reward, r_steps = self.rollout(shift=0.0, state_filter=state_filter)
...
class ARSLearner(object):
    ...
    def __init__(
        ...
        self.optimizer = optimizers.SGD(self.w_policy, self.learning_rate, self.lr_decay)
    ...
    def train(self, num_iter, state_filter=False):
        ...
        for i in range(num_iter):
            ...
            if (i + 1) % self.log_every == 0:
                rewards = self.aggregate_rollouts(num_rollouts=self.num_deltas, evaluate=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="Learning rate to start training at.",
    )
